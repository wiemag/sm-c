...
# Check /tmp/sm after mailx has been called directly or cron sent an email message.  #
...
# ---- Debugging part ------------------
	...
	echo "-------------------" >> /tmp/sm
	echo -n "Called from " >> /tmp/sm
	[[ -t 1 ]] && echo "a terminal" >> /tmp/sm || echo "not-a-terminal" >> /tmp/sm
	#[[ -t 1 || -t 0 ]] && echo "a terminal" >> /tmp/sm || echo "not a terminal" >> /tmp/sm
	echo "-------------------" >> /tmp/sm
...
# ---- Debugging part ------------------
	echo The arguments for msmtp: $TO >> /tmp/sm
